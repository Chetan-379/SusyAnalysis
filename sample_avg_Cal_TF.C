void sample_avg_Cal_TF()
{
  ...
  f = {"final_Summer20UL18_LL_All_Combined.root"};
  ...
  tf4 = {"final_2018_lost_e_TF_NBJet.root","final_2018_lost_mu_TF_NBJet.root","final_2018_LL_TF_NBJet.root"};
  ...
  for (int i_year =0; i_year < f.size(); i_year++){
      ...
      for(int i_hist=0; i_hist < cat[icat].size();i_hist++)
	{
      ...
      // cout << "===================TF for 2018==========================" << endl;
      // if (i_year ==3){	
      // 	if (icat==0) {cout << "TFs for lost e:" << endl;
      // 	  for (int ibin=0; ibin < h_TF->GetNbinsX(); ibin++){
      // 	    cout << "SR in Bin" << ibin << ": " << hist_list[0]->GetBinContent(ibin) << endl;
      // 	    cout << "CR in Bin" << ibin << ": " << hist_list[1]->GetBinContent(ibin) << endl;
