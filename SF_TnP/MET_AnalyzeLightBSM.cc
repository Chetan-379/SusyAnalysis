 ...
 void AnalyzeLightBSM::EventLoop(const char *data,const char *inputFileList, const char *sample , const char *outFileName,  const char* phoID) {
  ...
  //  // counters for events yields after each selection/rejection
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++)
    ...
    double mTPhoMET = sqrt(2*(bestEMObj.Pt())*new_MET*(1-cos(DeltaPhi(new_METPhi,bestEMObj.Phi()))));
    //double dPhi_PhoMET= abs(Met.DeltaPhi(bestEMObj));//DeltaPhi(METPhi,bestPhoton.Phi()));
    double dPhi_PhoMET= abs((Met+tag_Met).DeltaPhi(bestEMObj));//DeltaPhi(METPhi,bestPhoton.Phi()));
    ...
    dPhi_METjet1 = abs((Met + tag_Met).DeltaPhi(hadJets[0]));
    dPhi_METjet2 = abs((Met + tag_Met).DeltaPhi(hadJets[1]));
	...
	cout<<"in Electron CR ====== "<<jentry<<endl;
      mt_ele=sqrt(2*bestEMObj.Pt()*new_MET*(1-cos(DeltaPhi(new_METPhi,bestEMObj.Phi()))));
