...
void generate_1Dplot(vector<TH1F*> hist, char const *tag_name="", float energy=-1, int xmax=-1,int xmin=-1,char const *leg_head="",
  ...
  legend = new TLegend(0.68,0.75,0.83,0.9);  
  ...
  textOnTop->SetTextSize(0.035);
    ...
    //leg_entry0[i] = legend->AddEntry(hist.at(i),(hist.at(i)->Integral().c_str()),"xsl");
  ...
  for(int i = 0; i < (int)hist.size(); i++) {
    if(!normalize) hist.at(i)->GetYaxis()->SetRangeUser(1.0,ymax*10.0);
      ...
      hist.at(i)->GetYaxis()->SetRangeUser(1.0,ymax*60.0);
    //    p1->SetGrid();
    if (i>0) h_stack->Add(hist.at(i));
    h_stack->SetMinimum(0.01);
    h_stack->SetMaximum(ymax*1000);
  ...
  //hist.at(0) -> Draw("hist");
  h_stack -> Draw("HIST");
  h_stack->GetXaxis()->SetTitleOffset(1.0);
  hist.at(0) -> Draw("sames");
  gPad->Modified(); gPad->Update();
  //h_stack->GetXaxis()->SetTitle(xtitle);
  h_stack->GetXaxis()->SetRangeUser(xmin,xmax+0.01*xmax);
  //  h_stack->GetYaxis()->SetTitleSize(
  h_stack->GetYaxis()->SetTitle("Entries");//h_stack->GetYaxis()->SetTitle("Events");
  h_stack->SetTitle(0);
  h_stack->GetYaxis()->SetTitleOffset(1.2);
  h_stack->GetXaxis()->SetTitleSize(00.05);
  h_stack->GetXaxis()->SetLabelSize(0.04);
  h_stack->GetYaxis()->SetLabelSize(0.04);
  h_stack->GetYaxis()->SetTitleSize(00.055);
  h_stack->GetYaxis()->SetTitleOffset(1.0);

  //  for(int j=0;j<hist.size();j++){
  //   hist.at(j)->Draw("Hist same");
  // }
