...
void all_combined_Bin_NB_TF(string pathname)
{
  ...
  bool store_TF = false;
  if (!store_TF) histnames = {"lost_e_SR_srch_binned", "lost_e_SR_srch_binned_pred", "lost_e_CR_srch_binned"};
  if (store_TF) histnames = {"lost_e_SR_binned", "lost_e_CR_binned"};
  ...
  //   }
	...
	for(int i_file=0; i_file < f.size(); i_file++){ //looping over each file
	  ...
	  for(int i_hist=0; i_hist < histnames.size();i_hist++) //looping over different histograms which should be overlayed on the same canvas and these histograms are saved in the same file	    
	    {
	...
	if(!store_TF){            	      
	  ...
	  // for (int ibkg = 0; ibkg < f.size(); ibkg ++){
	    ...
	    TH1D* h_TF = (TH1D*)h_added.at(0)->Clone();
	    h_TF->Divide(h_added.at(1));
	    ...
	    sprintf(full_path,"%s/%sValidate_binned_SR_CR_overlay_%s",pathname.c_str(),folder.c_str(),filetag.c_str());
