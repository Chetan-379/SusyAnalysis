...
//TString legend_text[15] = {"TTJets","TTGJets"};
//TString legend_text[15] = {"WJets", "WGJets","pMSSM_MCMC_106_19786","pMSSM_MCMC_473_54451"};
//TString legend_text[15] = {"ZJets","ZGJets"};
TString legend_text[15] = {"QCD", "GJets"};
...
void generate_1Dplot(vector<TH1F*> hist, char const *tag_name="", float energy=-1, int xmax=-1,int xmin=-1,char const *leg_head="",
  ...
  legend = new TLegend(0.70,0.70,0.84,.9);  
  ...
  textOnTop->DrawLatexNDC(0.13,0.69,"it{#bf{Work in Progress}}");
  ...
  textOnTop->DrawLatexNDC(0.12,0.96," #it{#bf{Work in Progress}}");
...
void randomoverlay_dr(string pathname)
{
  ...
  ///f = {"root_files/Summer20UL18_TTJets.root","root_files/Summer20UL18_TTGJets_Tune.root"};
  //f = {"root_files/Summer20UL18_WJetsToLNu_HT.root","root_files/Summer20UL18_WGJets_MonoPhoton.root"};
  //f={"root_files/Summer20UL18_ZJetsToNuNu.root","root_files/Summer20UL18_ZNuNuGJets_MonoPhoton.root"};
  f={"root_files/Summer20UL18_QCD_HT.root","root_files/Summer20UL18_GJets_DR-0p4_HT.root"};
  ...
  for(int icat =0; icat < category.size(); icat++)
    {
      ...
      for(int i_cut=0; i_cut<category[icat].size();i_cut++) //looping over different histograms which should be overlayed on the same canvas and these histograms are saved in the same file
	{
	  ...
	  for(int i_file=0; i_file < f.size(); i_file++) //looping over each file
	    { TFile *root_file = new TFile(f[i_file].c_str()); 
	      ...
	      //   {
  	      ...
  	      sprintf(hist_name,"%s",category[icat][i_cut].c_str());
  	  ...
  	  diff_title = {"mindR(reco_pho,genLep)","mindR(gen_pho,q_g)"};//,"mindR(reco_pho,genLep)","mindR(madmindR)"};
	
	  //diff_title = {"mindR(reco_pho,q_g)"};
	  ...
	  //xtitle.push_back(diff_title[i_cut]);
	  ...
	  //folder = {"plots/TT_/","plots/TT_/"};
	  //folder = {"plots/cutflow_plots/ZNuNu_/dR_plots/","plots/cutflow_plots/ZNuNu_/dR_plots/"};
	  folder = {"plots/cutflow_plots/QCD_/dR_plots/","plots/cutflow_plots/QCD_/dR_plots/"};
	  //folder = {"plots/QCD_/","plots/QCD_/"};
  	  //sprintf(full_path,"%s/%s%s_mindR_overlay_%s",pathname.c_str(),folder[i_file].c_str(),diff_title[i_file].c_str(),filetag[i_file].c_str());
	  sprintf(full_path,"%s/%sQCDJets_Normalized_%s_%s",pathname.c_str(),folder[i_cut].c_str(),diff_title[i_cut].c_str(),filetag[icat].c_str());
